...
public:
  ...
  std::error_code put(Slice Key, Slice &Data, unsigned int Flag = 0);
...
class DupIterator : lmdb::Cursor {
  ...
  std::optional<Slice> prevNextImpl(::MDB_cursor_op Op) {
    assert(!ErrorCode);
    if (std::exchange(Fresh, false))
      Op = MDB_GET_CURRENT;
    Slice Key, Data;
...
public:
  ...
  std::optional<Slice> prev() { return prevNextImpl(MDB_PREV_DUP); }

  std::optional<Slice> next() { return prevNextImpl(MDB_NEXT_DUP); }
