...
#ifndef LLVM_CLANG_TOOLS_EXTRA_CLANGD_INDEX_LMDB_INDEX_BACKGROUND_H
#define LLVM_CLANG_TOOLS_EXTRA_CLANGD_INDEX_LMDB_INDEX_BACKGROUND_H
...
struct ShardVersion {
  ...
  bool operator==(const ShardVersion &R) {
    return (ModifiedTime ? (ModifiedTime == R.ModifiedTime)
                         : Digest == R.Digest) &&
...
public:
  ...
  BackgroundIndex(llvm::StringRef WorkspaceRoot, const ThreadsafeFS &,
                  const GlobalCompilationDatabase &CDB, Options Opts);
...
private:
  ...
  std::string DataStorageRoot;
  std::string ShardTmpDirPath;
  std::string LMDBDatabasePath;

  // Database
  std::shared_ptr<lmdb_index::DocumentStore> LMDBInstance;
