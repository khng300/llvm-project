...
#include "index/lmdb-index/BackgroundIndexLoader.h"
#include "index/lmdb-index/DocumentStore.h"
#include "index/lmdb-index/LMDBWrapperError.h"
...
bool shardIsStale(const LoadedShard &LS, llvm::vfs::FileSystem *FS) {
  auto StatusOrErr = FS->status(LS.AbsolutePath);
  if (!StatusOrErr || LS.ModifiedTime) {
  ...
  return llvm::sys::toTimeT(StatusOrErr->getLastModificationTime()) >
         LS.ModifiedTime;
}

std::string getDataStorageRootPath(llvm::StringRef WorkspaceRoot) {
  llvm::SmallString<128> DataStorageRootSS(WorkspaceRoot);
  if (DataStorageRootSS.empty()) {
    if (!llvm::sys::path::cache_directory(DataStorageRootSS))
      return nullptr;
    llvm::sys::path::append(DataStorageRootSS, "clangd");
  }
  llvm::sys::path::append(DataStorageRootSS, ".cache", "clangd");
  return std::string(DataStorageRootSS);
}

std::string getShardTmpDirPath(llvm::StringRef DataStorageRoot) {
  llvm::SmallString<128> ShardTmpDirSS = DataStorageRoot;
  llvm::sys::path::append(ShardTmpDirSS, "shards-tmp");
  return std::string(ShardTmpDirSS);
}

std::string getLMDBDatabasePath(llvm::StringRef DataStorageRoot) {
  llvm::SmallString<128> LMDBDatabasePathSS = DataStorageRoot;
  llvm::sys::path::append(LMDBDatabasePathSS, "db-index");
  return std::string(LMDBDatabasePathSS);
}

std::string getShardPathFromFilePath(llvm::StringRef DataStorageRoot,
                                     llvm::StringRef FilePath) {
  llvm::SmallString<128> ShardRootSS =
      llvm::StringRef(getShardTmpDirPath(DataStorageRoot));
...
BackgroundIndex::BackgroundIndex(llvm::StringRef WorkspaceRoot,
    ...
    : SwapIndex(std::make_unique<MemIndex>()), TFS(TFS), CDB(CDB),
      ...
      DataStorageRoot(getDataStorageRootPath(WorkspaceRoot)),
      ShardTmpDirPath(getShardTmpDirPath(DataStorageRoot)),
      LMDBDatabasePath(getLMDBDatabasePath(DataStorageRoot)),
      LMDBInstance(lmdb_index::createSharedDatabase(LMDBDatabasePath)),
      Rebuilder(this, LMDBInstance, Opts.ThreadPoolSize),
  ...
  if (auto EC = llvm::sys::fs::create_directories(ShardTmpDirPath))
    elog("Cannot create shard path tmp storage {0}: {1}", ShardTmpDirPath,
         EC.message());
}
...
void BackgroundIndex::update(
    ...
    bool HadErrors) {
  auto FS = Index.Cmd ? TFS.view(Index.Cmd->Directory) : TFS.view(std::nullopt);
  // Keys are URIs.
  llvm::StringMap<std::pair<Path, ShardVersion>> FilesToUpdate;
  ...
  for (const auto &IndexIt : *Index.Sources) {
    ...
    if (DigestIt == ShardVersionsSnapshot.end() ||
      ...
      auto StatusOrErr = FS->status(*AbsPath);
      if (StatusOrErr) {
        ModifiedTime =
            llvm::sys::toTimeT(StatusOrErr->getLastModificationTime());
      } else {
        ModifiedTime = 0;
      }
      FilesToUpdate[IGN.URI] = {
          std::move(*AbsPath),
          ShardVersion{IGN.Digest, ModifiedTime, HadErrors}};
    }
  }
  ...
  for (const auto &FileIt : FilesToUpdate) {
    ...
    ShardVersion Version = FileIt.getValue().second;

    std::lock_guard<std::mutex> Lock(ShardVersionsMu);
    const auto &Hash = FileIt.getValue().second.Digest;
    const auto ModifiedTime = Version.ModifiedTime;
    ...
    // and this one is not.
    if (!DigestIt.second && SV.ModifiedTime == ModifiedTime && SV.HadErrors &&
        !HadErrors)
      continue;
    SV.Digest = Hash;
    SV.ModifiedTime = ModifiedTime;
    SV.HadErrors = HadErrors;

    auto ShardPath = getShardPathFromFilePath(DataStorageRoot, Path);
    auto Uri = FileIt.first();
    auto ShardIndex = ShardedIndex.getShard(Uri);
    auto Model = ShardPath + ".tmp-%%%%%%";
    auto ShardTmpFile = llvm::sys::fs::TempFile::create(Model);
    if (!ShardTmpFile) {
      elog("Cannot create shard path {0}: {1}", Model,
           ShardTmpFile.takeError());
      continue;
    }
    ShardQueueItem QI{
        std::move(*ShardTmpFile),
        std::chrono::system_clock::now().time_since_epoch().count(), Path.str(),
        MainFile.str()};
    llvm::raw_fd_ostream Out(QI.ShardTmpFile.FD, false);
    Out << IndexFileOut(*ShardIndex);
    Rebuilder.queueTUShard(std::move(QI));
  }
}
...
BackgroundIndex::loadProject(std::vector<std::string> MainFiles) {
  ...
  const std::vector<LoadedShard> Result =
      loadIndexShards(MainFiles, *LMDBInstance, CDB);
  size_t LoadedShards = 0;
  {
    ...
    for (auto &LS : Result) {
      ...
      SV.Digest = LS.Digest;
      SV.ModifiedTime = LS.ModifiedTime;
