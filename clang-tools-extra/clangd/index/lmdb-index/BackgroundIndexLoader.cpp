...
public:
  BackgroundIndexLoader(lmdb_index::DocumentStore &IndexStorage)
...
private:
  ...
  lmdb_index::DocumentStore &IndexStorage;
};
...
BackgroundIndexLoader::loadShard(PathRef StartSourceFile, PathRef DependentTU) {
  ...
  auto Shard = IndexStorage.loadShardInfo(Txn, StartSourceFile);
  if (!Shard) {
    elog("Failed to load shard: {0}", StartSourceFile);
    return {LS, Edges};
  } else {
    log("Loaded shard: {0}", StartSourceFile);
  }
