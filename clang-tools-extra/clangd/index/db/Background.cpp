...
#include "index/db/BackgroundIndexLoader.h"
#include "index/db/IndexDBError.h"
...
BackgroundIndex::BackgroundIndex(llvm::StringRef WorkspaceRoot,
    ...
    : SwapIndex(std::make_unique<MemIndex>()), TFS(TFS), CDB(CDB),
      ...
      LMDBInstance(db_index::createSharedDatabase(WorkspaceRoot)),
...
void BackgroundIndex::update(
  ...
  do {
    size_t DBSize = LMDBInstance->databaseSize();
    CommitEC = [&]() {
      auto TxnOrErr = LMDBInstance->beginTransaction(false);
      if (!TxnOrErr)
      ...
      auto EC = [&]() -> std::error_code {
        ...
        for (const auto &FileIt : FilesToUpdate) {
          ...
          if (auto Error = LMDBInstance->updateShard(Txn, Path, &IOut,
    ...
    if (CommitEC) {
      if (CommitEC != IndexDBError::MapFull) {
        elog("BackgroundIndex::update() error: {0}", CommitEC.message());
        llvm::report_fatal_error("BackgroundIndex::update() failed");
      }
      auto EC = LMDBInstance->growDatabaseSize(DBSize);
      if (EC) {
        elog("Instance->growSize() error: {0}", EC.message());
        llvm::report_fatal_error("BackgroundIndex::update() failed");
      }
    }
  } while (CommitEC == IndexDBError::MapFull);
