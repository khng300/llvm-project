...
struct DatabaseIndexCursor {
  ...
  std::optional<OID> getOID() {
    return getSetCursor(lmdb::DBOperation::GetCurrent);
  }

  std::optional<OID> gotoOID(OID Oid) {
    llvm::StringRef Key, Data;
    auto Result = getSetCursor(Key, Data, lmdb::DBOperation::GetCurrent);
    if (!Result)
      return std::nullopt;
  ...
  std::optional<OID> prev() { return getSetCursor(lmdb::DBOperation::PrevDup); }
  std::optional<OID> next() { return getSetCursor(lmdb::DBOperation::NextDup); }
  ...
  bool statusOk() { return Cursor.valid(); }
  std::error_code statusErrorCode() const { return StatusErrorCode; }
...
private:
  void moveAssign(DatabaseIndexCursor &&It) noexcept {
    ...
    std::swap(It.StatusErrorCode, StatusErrorCode);
  }

  std::optional<OID> getSetCursor(llvm::StringRef &Key, llvm::StringRef &Data,
    ...
    return llvm::bit_cast<OID>(*reinterpret_cast<const OID *>(Data.data()));
  }

  std::optional<OID> getSetCursor(lmdb::DBOperation Operation) {
    llvm::StringRef Key, Data;
