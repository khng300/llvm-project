...
public:
  BackgroundIndexLoader(db_index::LMDB &IndexStorage)
...
BackgroundIndexLoader::loadShard(PathRef StartSourceFile, PathRef DependentTU) {
  ...
  auto TxnOrErr = IndexStorage.beginTransaction(true);
  if (!TxnOrErr) {
    vlog("Failed to start trasnsaction to load shard: {0}", StartSourceFile);
    return {LS, Edges};
  }
  auto &Txn = *TxnOrErr->get();
  auto Shard = IndexStorage.getShardInfo(Txn, StartSourceFile);
  if (!Shard) {
    vlog("Failed to load shard: {0}", StartSourceFile);
    return {LS, Edges};
  }

  LS.Shard = std::make_unique<IncludeGraph>(std::move(*Shard->Sources));
  LS.ModifiedTime = Shard->ModifiedTime;
  for (const auto &It : *LS.Shard) {
...
std::vector<LoadedShard> loadIndexShards(llvm::ArrayRef<Path> MainFiles,
                                         db_index::LMDB &IndexStorage,
                                         const GlobalCompilationDatabase &CDB) {
  BackgroundIndexLoader Loader(IndexStorage);
