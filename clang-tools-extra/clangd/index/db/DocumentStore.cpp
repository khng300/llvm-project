...
public:
  ...
  void advance() override {
    ...
    auto OidResult = IndexCursor.next();
    IsEnd = !OidResult;
  }
...
bool LMDBInstance::QueryRequest::refs(
  ...
  for (const auto &ID : Req.IDs) {
    auto ItOrErr = Snapshot->findReferences(ID);
    if (!ItOrErr) {
      if (auto EC = ItOrErr.getError(); EC != IndexDBError::Notfound) {
        elog("{0}: Error: {1}", __func__, EC.message());
        break;
      }
      continue;
    }
    auto &It = *ItOrErr;
    for (auto OidResult = It.getOID(); OidResult; OidResult = It.next()) {
      auto SR = DB->ShardRefsDB.readOid(Snapshot->getTxn(), *OidResult);
      if (!SR)
        continue;
    ...
    if (!It.statusOk()) {
      elog("{0}: Error: {1}", __func__, It.statusErrorCode().message());
      break;
    }
  }
  return false;
...
void LMDBInstance::QueryRequest::relations(
  ...
  for (const auto &ID : Req.Subjects) {
    auto ItOrErr = Snapshot->findRelations(ID, Req.Predicate);
    if (!ItOrErr) {
      if (auto EC = ItOrErr.getError(); EC != IndexDBError::Notfound) {
        elog("{0}: Error: {1}", __func__, EC.message());
        return;
    ...
    auto &It = *ItOrErr;
    for (auto OidResult = It.getOID(); OidResult; OidResult = It.next()) {
      auto Rel = DB->ShardRelationsDB.readOid(Snapshot->getTxn(), *OidResult);
      if (Remaining == 0)
        return;
      --Remaining;
...
LMDBInstance::TransactionImpl::findIndexFileInfo(llvm::StringRef ShardPath) {
  ...
  auto &It = *ItOrErr;
  for (auto OidResult = It.getOID(); OidResult; OidResult = It.next()) {
    auto Shard = DB->ShardDB.readOid(Txn, *OidResult);
    if (Shard && Shard->ShardPath == ShardPath)
      return std::pair{*OidResult, *Shard};
  }
  if (!It.statusOk())
    return It.statusErrorCode();
...
LMDBInstance::TransactionImpl::lookupShardsSymbol(SymbolID ID) {
  ...
  auto &It = *ItOrErr;
  for (auto OidResult = It.getOID(); OidResult; OidResult = It.next()) {
    auto ShardSym = DB->ShardSymbolsDB.readOid(Txn, *OidResult);
    if (!ShardSym)
...
std::error_code LMDBInstance::TransactionImpl::updateSymbol(SymbolID ID) {
  ...
  if (!SymOrErr) {
    if (auto EC = ItOrErr.getError(); EC != IndexDBError::Notfound)
      return EC;
    return DB->IndexedSymbolsDB.removeOid(Txn, *SymOid);
  }
  auto &Sym = *SymOrErr;
  ...
  return DB->IndexedSymbolsDB.writeOid(Txn, *SymOid, IndexedSym);
}
...
LMDBInstance::TransactionImpl::updateShard(llvm::StringRef ShardPath,
  ...
  } else if (auto EC = PrevShard.getError(); EC != IndexDBError::Notfound) {
    return EC;
  }
