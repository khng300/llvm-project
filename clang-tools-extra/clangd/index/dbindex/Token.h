...
#ifndef LLVM_CLANG_TOOLS_EXTRA_CLANGD_DBINDEX_TOKEN_H
#define LLVM_CLANG_TOOLS_EXTRA_CLANGD_DBINDEX_TOKEN_H
...
public:
  ...
  uint64_t getPersistentHash() const {
    return xxHash64(XxHashInitialSeed, static_cast<uint32_t>(TokenKind),
                    llvm::StringRef(Data));
  }
...
template <> struct DenseMapInfo<clang::clangd::dbindex::Token> {
  static inline clang::clangd::dbindex::Token getEmptyKey() {
    return {clang::clangd::dbindex::Token::Kind::Sentinel, "EmptyKey"};
  }

  static inline clang::clangd::dbindex::Token getTombstoneKey() {
    return {clang::clangd::dbindex::Token::Kind::Sentinel, "TombstoneKey"};
  }

  static unsigned getHashValue(const clang::clangd::dbindex::Token &Tag) {
    return hash_value(Tag);
  }

  static bool isEqual(const clang::clangd::dbindex::Token &LHS,
                      const clang::clangd::dbindex::Token &RHS) {
    return LHS == RHS;
  }
};
...
#endif // LLVM_CLANG_TOOLS_EXTRA_CLANGD_DBINDEX_TOKEN_H
