...
public:
  ...
  explicit operator bool() { return Status.operator bool(); }
  bool hasError() { return !operator bool(); }

  bool isHit() { return conditionEqual(Condition::Hit); }
  bool isMiss() { return conditionEqual(Condition::Miss); }
...
private:
  ...
  bool conditionEqual(Condition RHS) {
    return !Status.errorIsA<Error>() && *Status == RHS;
  }
};
