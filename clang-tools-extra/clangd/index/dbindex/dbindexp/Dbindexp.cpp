...
int main(int argc, const char *argv[]) {
  ...
  auto LmdbIndex = dbindex::LMDBIndex::open(IndexPath);
  if (!LmdbIndex) {
    llvm::outs() << "Failed to open the LMDBIndex database. Message: "
                 << LmdbIndex.takeError() << "\n";
    return -1;
  }

  std::unique_ptr<SymbolIndex> Index;
  reportTime("DbIndex build", [&]() {
    if (auto Idx = (*LmdbIndex)->buildIndex())
      Index = std::move(*Idx);
    else
      llvm::outs() << "Error occured. Message: " << Idx.takeError() << "\n";
  });

  if (!Index) {
    llvm::outs() << "Failed to build the DbIndex.\n";
    return -1;
  }

  llvm::LineEditor LE("dbindexp");
